...
function DevModeBadge({ dark }: { dark: boolean }) {
  ...
  return (
    <div className="relative">
      ...
      {show && (
        <div className="absolute right-0 top-8 z-30 rounded-xl p-3 shadow-2xl w-56"
          ...
          <div className="flex items-center gap-1.5 text-[11px]" style={{ color: dark ? "rgba(255,255,255,0.70)" : "#374151" }}>
          ...
          <p className="text-[10px] mt-2" style={{ color: dark ? "rgba(255,255,255,0.45)" : "#6b7280" }}>
            Adicione, edite ou remova canais. Os dados aparecem aqui em tempo real para todos os usuários.
          </p>
        </div>
      )}
    </div>
...
function ChannelCard({ canal, dark, active, onToggle, onDetail }: {
  ...
  return (
    <div
      ...
      style={{
        background: active ? (dark ? "rgba(255,255,255,0.04)" : "rgba(255,255,255,0.95)") : (dark ? "rgba(255,255,255,0.02)" : "rgba(0,0,0,0.02)"),
      ...
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 rounded-lg flex items-center justify-center text-[10px] font-black text-white flex-shrink-0"
            style={{ background: canal.cor }}>{canal.icone}</div>
          <div>
          ...
          <div className="flex items-start justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className={`text-xs mt-0.5 ${dark ? "text-white/45" : "text-gray-500"}`}>{canal.tipo} · {canal.descricao}</p>
              </div>
            </div>
            <button onClick={onClose} className={`p-2 rounded-xl ${dark ? "bg-white/10 text-white" : "bg-gray-100 text-gray-600"}`}>
              <X size={16} />
            </button>
          </div>
        </div>
        <div className="p-6 space-y-6">
          ...
          <div>
            ...
            <div className="h-2.5 rounded-full overflow-hidden" style={{ background: dark ? "rgba(255,255,255,0.06)" : "rgba(0,0,0,0.06)" }}>
              <div className="h-full rounded-full" style={{ width: `${pct}%`, background: `linear-gradient(90deg,${canal.cor}80,${canal.cor})` }} />
            </div>
          </div>
          <div>
            <p className={`text-xs font-bold mb-3 ${dark ? "text-white/55" : "text-gray-600"}`}>Evolução Mensal — {canal.unidade}</p>
            <ResponsiveContainer width="100%" height={180}>
              <ComposedChart data={canal.dados_mensais} margin={{ top: 5, right: 10, bottom: 0, left: 0 }}>
                ...
                {canal.meta_anual > 0 && (
                  <ReferenceLine y={canal.meta_anual} stroke={canal.cor} strokeDasharray="5 4" strokeOpacity={0.4}
                    label={{ value: `Meta: ${canal.meta_anual}`, position: "right", fontSize: 10, fill: canal.cor }} />
                )}
                <Area type="monotone" dataKey="valor" name={canal.nome} stroke={canal.cor} fill={canal.cor + "18"} strokeWidth={2.5}
                  dot={false} activeDot={{ r: 5, strokeWidth: 2, stroke: dark ? "#0a1628" : "#fff", fill: canal.cor }} />
              </ComposedChart>
            </ResponsiveContainer>
          </div>
          <div>
            <p className={`text-xs font-bold mb-3 ${dark ? "text-white/55" : "text-gray-600"}`}>Breakdown Trimestral</p>
          ...
          <div>
            <p className={`text-xs font-bold mb-3 ${dark ? "text-white/55" : "text-gray-600"}`}>Dados Mensais Completos</p>
          ...
          {/* Dev Mode path indicator */}
          <div className="rounded-xl p-3 flex items-center gap-3"
            style={{ background: "rgba(59,130,246,0.08)", border: "1px solid rgba(59,130,246,0.20)" }}>
            <Info size={14} className="text-blue-400 flex-shrink-0" />
            <div>
              <p className="text-[10px] font-bold text-blue-400 mb-0.5">Para editar este canal</p>
              <div className="flex items-center gap-1.5">
                ...
                <ArrowRight size={9} className="text-blue-400" />
                <span className="text-[10px] text-blue-300">{canal.nome}</span>
              </div>
            </div>
          </div>
        </div>
...
export default function ChannelChart({ dark = true }: Props) {
  ...
  const chartData = canaisAtivos[0]?.dados_mensais.map((_, mi) => {
    const ponto: Record<string, any> = { mes: canaisAtivos[0].dados_mensais[mi].mes }
    canaisAtivos.forEach(c => { ponto[c.nome] = c.dados_mensais[mi]?.valor || 0 })
    return ponto
  }) || []
  ...
  if (canaisAtivos.length === 0) {
    return (
      <div className="rounded-2xl p-8 text-center" style={{ background: cardBg, border: `1px dashed ${cardBorder}` }}>
  ...
  return (
    <div className="space-y-5">
      {/* Header com badge Dev Mode */}
      <div className="flex items-center justify-between">
        <div>
          <p className={`text-xs font-semibold uppercase tracking-widest ${dark ? "text-white/30" : "text-gray-400"}`}>
            Evolução de Seguidores
          </p>
          <p className={`text-[10px] mt-0.5 ${dark ? "text-white/20" : "text-gray-300"}`}>
            Dados sincronizados com Firestore · visível para todos os usuários
          </p>
        </div>
      ...
      {tipos.map(tipo => (
        <div key={tipo}>
          <p className={`text-[11px] font-semibold uppercase tracking-widest mb-3 ${dark ? "text-white/30" : "text-gray-400"}`}>{tipo}</p>
          <div className="flex gap-3 flex-wrap">
            {canaisAtivos.filter(c => c.tipo === tipo).map(canal => (
              <ChannelCard key={canal.id} canal={canal} dark={dark}
      ...
      <div className="rounded-2xl p-5" style={{ background: cardBg, border: `1px solid ${cardBorder}` }}>
        <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
          <div>
            <p className={`font-bold text-sm ${dark ? "text-white" : "text-gray-900"}`}>Evolução por Canal — 2026</p>
          ...
          <div className="flex gap-3 flex-wrap">
            {canaisAtivos.map(c => (
              <div key={c.id} className="flex items-center gap-1.5 cursor-pointer transition-opacity"
          ...
          <ComposedChart data={chartData} margin={{ top: 10, right: 20, bottom: 0, left: 0 }}>
            ...
            {canaisAtivos.map(c => allActive[c.id] && (
              <Area key={c.id} type="monotone" dataKey={c.nome}
                ...
                dot={false} activeDot={{ r: 6, strokeWidth: 2, stroke: dark ? "#070f1f" : "#ffffff", fill: c.cor }} />
            ))}
