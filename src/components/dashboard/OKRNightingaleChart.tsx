      ...
      <div className="px-6 pt-6 pb-2 flex items-start justify-between">
        ...
        <div className="flex gap-1.5 mt-1">
          {COLORS.slice(0, 4).map((c, i) => (
            <div key={i} className="w-3.5 h-3.5 rounded-sm" style={{ background: `linear-gradient(135deg, ${c.from}, ${c.to})` }} />
          ))}
        </div>
      </div>

      <div className="relative flex justify-center overflow-hidden" onMouseLeave={handleMouseLeave}>
        <svg
          width="580" height="510" viewBox="0 0 580 510"
        ...
        >
          ...
          {[1, 0.65, 0.35].map((r, i) => (
            <circle key={i} cx={cx} cy={cy} r={maxR * r} fill="none" stroke="rgba(255,255,255,0.06)" strokeWidth={1} strokeDasharray="4 5" />
          ))}
          ...
          {data.map((m, i) => {
            ...
            return <path key={`sh${i}`} d={slicePath(cx + 2, cy + 5, r, a0, a1)} fill={COLORS[i].to} opacity={0.25} />
          })}
          ...
          {data.map((m, i) => {
            ...
            return (
              <g key={i}>
                <path d={slicePath(cx, cy, scaleR, a0, a1)} fill={`url(#rg${i})`} opacity={isHov ? 1 : 0.9} filter={isHov ? "url(#glow)" : "url(#shadow)"} style={{ cursor: "pointer", transition: "opacity 0.2s" }} onMouseEnter={() => handleMouseEnterSlice(i)} />
                <path d={slicePath(cx, cy, scaleR, a0, a0 + 0.025)} fill="rgba(255,255,255,0.25)" pointerEvents="none" />
                <path d={slicePath(cx, cy, scaleR * 0.45, a0 + 0.04, a1 - 0.04)} fill="rgba(255,255,255,0.07)" pointerEvents="none" />
                <line x1={tipP.x.toFixed(1)} y1={tipP.y.toFixed(1)} x2={dotP.x.toFixed(1)} y2={dotP.y.toFixed(1)} stroke={isHov ? COLORS[i].accent : "rgba(255,255,255,0.22)"} strokeWidth={isHov ? 1.2 : 0.7} />
                <circle cx={dotP.x} cy={dotP.y} r={2.5} fill={isHov ? COLORS[i].accent : "rgba(255,255,255,0.38)"} />
                <text x={labelP.x.toFixed(1)} y={(labelP.y - 5).toFixed(1)} textAnchor={anchor} fill={COLORS[i].accent} fontSize="14" fontWeight="800" fontFamily="system-ui, sans-serif">{m.value}%</text>
                <text x={labelP.x.toFixed(1)} y={(labelP.y + 7).toFixed(1)} textAnchor={anchor} fill="rgba(255,255,255,0.72)" fontSize="7" fontWeight="700" fontFamily="system-ui, sans-serif" letterSpacing="0.5">{m.label.toUpperCase()}</text>
                <text x={labelP.x.toFixed(1)} y={(labelP.y + 17).toFixed(1)} textAnchor={anchor} fill="rgba(255,255,255,0.35)" fontSize="6.5" fontFamily="system-ui, sans-serif">{m.sublabel}</text>
              </g>
            )
          })}
        ...
        {hoveredSlice !== null && (
          ...
          >
            <div className="flex items-center gap-1.5 mb-1">
              <div className="w-2.5 h-2.5 rounded-full flex-shrink-0" style={{ background: `linear-gradient(135deg,${COLORS[hoveredSlice].from},${COLORS[hoveredSlice].to})` }} />
            ...
            <div className="text-white/60 text-[11px] mt-1 font-medium">
              Meta: <span style={{ color: COLORS[hoveredSlice].accent }}>{data[hoveredSlice].sublabel}</span>
            </div>
          </div>
        )}
      </div>
          ...
          {data.map((m, i) => (
            <div key={i} className="group">
              <div className="flex items-center justify-between mb-1.5 gap-1">
                {editingBar === i ? (
                  <input value={m.label} onChange={e => updateLabel(i, 'label', e.target.value)} className="flex-1 text-[9px] font-semibold bg-transparent border-b border-white/20 text-white/60 outline-none" />
                ) : (
                ...
                <div className="flex items-center gap-0.5 flex-shrink-0">
                  {editingBar === i ? (
                    <>
                      <input type="number" min="0" max="100" value={editVal} onChange={e => setEditVal(e.target.value)} onKeyDown={e => e.key === 'Enter' && confirmEdit(i)} className="w-10 text-center text-[9px] font-extrabold bg-transparent border-b outline-none" style={{ color: COLORS[i].accent, borderColor: COLORS[i].from + '80' }} autoFocus />
                      <span className="text-white/30 text-[8px]">%</span>
                      <button onClick={() => confirmEdit(i)} className="text-green-400 hover:text-green-300 ml-0.5"><Check size={9} /></button>
                    </>
                  ) : (
                    <>
                      <span style={{ color: COLORS[i].accent }} className="text-[9px] font-extrabold">{m.value}%</span>
                      <button onClick={() => startEdit(i)} className="opacity-0 group-hover:opacity-100 transition-opacity ml-0.5">
              ...
              <div className="h-1 rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.08)" }}>
                <div className="h-full rounded-full transition-all duration-500" style={{ width: `${m.value}%`, background: `linear-gradient(90deg, ${COLORS[i].from}, ${COLORS[i].to})` }} />
              </div>
            </div>
          ))}
        </div>
