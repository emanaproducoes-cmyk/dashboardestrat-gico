...
function StatCard({ s, i, onUpdateValue, onUpdateLabel }: {
  s: StatItem, i: number,
  ...
  return (
    ...
    >
      ...
      >
        <EditableField value={s.value} onChange={onUpdateValue} className="font-extrabold" />
      </div>
      <p
        className="text-sm mt-1 font-medium"
        style={{
          color: hovered ? statAccents[i] : 'rgba(255,255,255,0.75)',
          transition: 'color 0.2s',
        }}
      >
        <EditableField value={s.label} onChange={onUpdateLabel} className="text-sm font-medium" />
      </p>
    </div>
  )
}
