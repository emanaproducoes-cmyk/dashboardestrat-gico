...
import { Sparkles, Clock, Pencil, Check, Camera, ShieldCheck } from "lucide-react"
...
function EditableField({ value, onChange, className = "", multiline, isAdmin }: EditableFieldProps) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 ${isAdmin ? "group/field cursor-pointer" : "cursor-default"}`}
    ...
    >
      ...
      {isAdmin && (
        <Pencil
          size={12}
          className="text-white/40 group-hover/field:text-white/80 transition-colors opacity-0 group-hover/field:opacity-100"
        />
      )}
    </span>
  )
...
export default function EditableHeroHeader({ accentGradient, isAdmin = false }: Props) {
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl p-8 text-white" style={{ background: gradientCss }}>
      ...
      {isAdmin && (
        <div className="absolute top-3 left-1/2 -translate-x-1/2 z-10 flex items-center gap-1.5 px-3 py-1 rounded-full bg-yellow-400/20 border border-yellow-300/40">
          ...
          <span className="text-[10px] font-semibold text-yellow-200">
            Modo edição ativo — clique nos textos para editar
          </span>
        </div>
      )}
      ...
      <div className={`relative flex items-start justify-between gap-4 ${isAdmin ? "mt-6" : "mt-0"}`}>
        <div className="flex items-center gap-4 min-w-0">
          <div
            className={`w-16 h-16 rounded-full bg-white/20 backdrop-blur-sm flex items-center justify-center ring-4 ring-white/30 relative group/avatar overflow-hidden flex-shrink-0 ${isAdmin ? "cursor-pointer" : "cursor-default"}`}
      ...
      <div className="relative mt-8 pt-6 border-t border-white/15 flex flex-wrap gap-3 md:gap-0 md:divide-x divide-white/20">
        {stats.map((s, i) => (
          <div
            key={i}
            className="md:flex-1 md:text-center px-4 first:pl-0 group/stat transition-transform duration-200 hover:scale-105"
          >
            <div className="text-3xl md:text-4xl font-extrabold tracking-tight leading-none">
              <EditableField
                value={s.value}
                onChange={v => updateStat(i, "value", v)}
                className="text-3xl md:text-4xl font-extrabold text-white transition-colors duration-200 group-hover/stat:text-yellow-300"
                isAdmin={isAdmin}
              />
            </div>
            <p className="text-[11px] text-blue-200 mt-1">
              <EditableField
                ...
                className="text-[11px] text-blue-200 transition-colors duration-200 group-hover/stat:text-white"
                isAdmin={isAdmin}
              />
            </p>
          </div>
